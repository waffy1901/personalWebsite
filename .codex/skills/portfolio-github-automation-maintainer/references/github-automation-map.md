...
| npm audit | `.github/workflows/npm-audit.yml` | Enforces the repository's npm audit policy on dependency PRs, weekly, and manually. | Production dependencies remain strict at moderate severity; the full-tree check permits only the documented dev-only `GHSA-mh99-v99m-4gvg` exception. |
