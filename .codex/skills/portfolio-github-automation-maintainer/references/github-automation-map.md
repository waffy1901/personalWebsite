...
| --- | --- | --- | --- |
| Main PR CI | `.github/workflows/dev-ci.yml` | Runs install, lint, tests, and build for PRs to `main`. | Uses Node `22.22.3`, `npm ci`, and `main/package-lock.json` cache path. |
...
| Deploy release | `.github/workflows/release-on-deploy.yml` | Verifies the app, waits for the exact Netlify production deploy, blocks release creation on live route/header/CSP/artifact contracts, then runs advisory browser smoke tests. | Requires `NETLIFY_AUTH_TOKEN` and `NETLIFY_SITE_ID`; preserves the legacy Netlify-domain check, serializes releases by ref, and keeps telemetry-safe Playwright non-blocking until it proves stable. |
| Semantic release | `.github/workflows/semantic-release.yml` | Publishes a curated `vMAJOR.MINOR.PATCH` release only from `main` for an exact ready production commit. | Manual dispatch requires the core package version and 40-character SHA; it verifies `main` ancestry, package-lock consistency, current ready Netlify provenance, deployment release, collisions, and monotonic versioning before publishing. |
| npm audit | `.github/workflows/npm-audit.yml` | Enforces the repository's npm audit policy on dependency PRs, weekly, and manually. | Production and full-tree checks fail directly on every moderate-or-higher vulnerability; no advisory exceptions remain. |
| Production health | `.github/workflows/deployed-security-headers.yml` | Resolves Netlify's current ready production commit, checks out that exact revision, and validates live routes, security headers, CSP, and public artifacts. | Keep the historical file path, the independent header-policy allowlist, exact `netlify.toml` comparison, and read-only Netlify access. |
