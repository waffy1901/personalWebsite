...
**Positioning:** Software Engineer II focused on platform reliability, Kubernetes workload automation, Cloud Spanner migrations, secure cloud identity, and high-throughput production systems.
...
**Stack snapshot:** React 19, React Router 8, Vite 8, Tailwind CSS 4, Vitest 4, ESLint 9.
...
- Node.js 22.22.2 or later (the checked-in workflows and Netlify build pin 22.22.3)
- npm
...
| `npm run start` | Alias for the local Vite development server |
| `npm run generate:public` | Regenerate public AI/discovery artifacts and README snapshots |
| `npm run generate:resume` | Regenerate canonical resume assets from the resume source |
...
- CodeQL analysis
- Deployed route, security-header, CSP, and public-artifact verification
...
After the required checks pass, the release workflow waits for the corresponding
Netlify production deployment, then blocks release creation on live route,
security-header, CSP, and public-artifact validation. A telemetry-safe Playwright
job runs after release creation as an advisory check; its failure uploads browser
evidence but does not currently invalidate the release. Keep that lane advisory
until five consecutive production deployments pass without a retry-only success
or another flaky result, then consider making it blocking.
